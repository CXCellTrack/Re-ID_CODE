function [phi_x_z, phi_x_zstar, loss] = MatchLift_cal_phi_and_loss( vars_ADN_F, fe_to_use, gt_connect, truth_num, n_person_cum )

% 解除vars
vars = mat2bolckmat( vars_ADN_F.vars, n_person_cum ); % 这样应该是无效的??
...
for i=1:n_seq-1
    for j=i+1:n_seq
        % 注意这里求出的vars不是0-1，计算误差时是否可以round??（不行，round之后变为非线性，无法计算）
        FN_num = FN_num + sum(sum(gt_connect{i,j}.*(1-vars{i,j})));
    end
end
    ...
    for j=i+1:n_seq
        ...
        for i_p1=1:n_p1
            for i_p2=1:n_p2
                ...
                phi_x_z = phi_x_z + vars{i,j}(i_p1,i_p2) * feature_diff{i,j}{i_p1,i_p2};
